...
DUGKS::DUGKS(MESO::ArgParser &parser, Config &config) : BasicSolver(parser, config) {
    /// params
    ...
    logger.note << "Solver::DUGKS loaded." << std::endl;
}
...
MESO::Scalar DUGKS::f_maxwell(Scalar rho, const Vector &u, const Vector &particle_velocity) const {
...
void DUGKS::fvm_update() {
    ...
    /// Update f_tilde_n+1
    auto C_bp_to_t = (dt + half_dt) / (half_dt - 2.0 * tau);
    for (auto &cell: mesh.cells) {
        ...
        for (int p = 0; p < mpi_task.size; ++p) {
            ...
            auto &particle = dvs_mesh.cells[dvs_id];
            auto f_eq = f_maxwell(rho, u, particle.position);
            auto f_bp = f_cell[p][cell.id];
            auto f_t = (1.0 - C_bp_to_t) * f_bp + C_bp_to_t * f_eq;
            f_cell[p][cell.id] = (4.0 * f_bp - f_t) / 3.0 - dt_v * flux_f[p][cell.id];
        }
    }
    /// Update Cell Macro Vars
    auto m0_local = mesh.zero_scalar_field();
    auto m1_local = mesh.zero_vector_field();
    for (auto &cell: mesh.cells) {
        for (int p = 0; p < mpi_task.size; ++p) {
    ...
    MPI::AllReduce(m0_local, m0);
    MPI::AllReduce(m1_local, m1);
    auto C_t_to_bp = (dt + half_dt) / (2.0 * tau + dt);
    for (auto &cell: mesh.cells) {
        auto rho = m0[cell.id];
        auto rhoU = m1[cell.id];
        auto u = rhoU / rho;
        ...
        for (int p = 0; p < mpi_task.size; ++p) {
            ...
            auto &particle = dvs_mesh.cells[dvs_id];
            auto f_eq = f_maxwell(rho, u, particle.position);
            f_cell[p][cell.id] = (1.0 - C_t_to_bp) * f_cell[p][cell.id] + C_t_to_bp * f_eq;
        }
    }
}
