...
void MESO::MPI::GatherField(Field<MESO::Scalar> &local, Field<MESO::Scalar> &global) {
    auto &mesh = *global.get_mesh();
    auto &group = mesh.cell_partition_groups[MPI::rank];
    for (auto cell_id : group) {
        global[cell_id] = local[cell_id];
        MPI_Bcast(&global[cell_id], 1, MPI_DOUBLE, MPI::rank, MPI_COMM_WORLD);
    }
    MPI_Barrier(MPI_COMM_WORLD);
}

void MESO::MPI::GatherField(Field<MESO::Vector> &local, Field<MESO::Vector> &global) {
    auto &mesh = *global.get_mesh();
    auto &group = mesh.cell_partition_groups[MPI::rank];
    for (auto cell_id : group) {
        global[cell_id] = local[cell_id];
        MPI_Bcast(&global[cell_id], 1, UDF::MPI_Vector, MPI::rank, MPI_COMM_WORLD);
    }
    MPI_Barrier(MPI_COMM_WORLD);
}
